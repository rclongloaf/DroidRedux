...
internal class TimeTravelControllerImpl(private val scope: CoroutineScope) : TimeTravelController {
    private var eventId = 1L
    private val stateSubject = MutableStateFlow(TimeTravelState())
    ...
    override fun states(observer: Observer<TimeTravelState>): Disposable {
...
//            logE("Could not enable time travel for the store: $name. Duplicate store name.")
    ...
    private fun addStore(store: TimeTravelStore<*>, name: String) {
        ...
        scope.launch {
            store.events
                .onEach { onEvent(TimeTravelEvent(id = eventId++, storeName = name, type = it.type, value = it.value, state = it.state)) }
                .onCompletion { stores -= name }
                .collect()
        }
    }
...
//        }
    ...
    private inline fun swapState(reducer: (TimeTravelState) -> TimeTravelState) {
        stateSubject.tryEmit(reducer(state))
    }
}
